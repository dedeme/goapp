...
func prGet(m *machine.T) {
	...
	v, ok := mp[k]
	if !ok {
		m.Fail("Map error", "Key '%v' not found", k)
	}
	m.Push(v)
}
...
func eq(
	m *machine.T, run func(m *machine.T), p []*token.T,
...
func prEq(m *machine.T, run func(m *machine.T)) {
	...
	mp1 := popMap(m)
	mp2 := popMap(m)
	m.Push(token.NewB(eq(m, run, p, mp1, mp2), m.MkPos()))
}
...
func prKeys(m *machine.T) {
	...
	var l []*token.T
	for k := range mp {
		l = append(l, token.NewS(k, m.MkPos()))
	}
...
func prPairs(m *machine.T) {
	...
	var l []*token.T
	for k, v := range mp {
		l = append(l, token.NewL([]*token.T{token.NewS(k, pos), v}, pos))
	}
	m.Push(token.NewL(l, m.MkPos()))
}
...
func prTo(m *machine.T) {
	...
	var l []*token.T
	for k, v := range mp {
		l = append(l, token.NewS(k, m.MkPos()), v)
	}
	m.Push(token.NewL(l, m.MkPos()))
}
...
func Proc(m *machine.T, run func(m *machine.T)) {
	...
	sy, ok := tk.Sy()
	if !ok {
		m.Failt("Expected: 'list' procedure.\nActual  : '%v'.", tk.StringDraft())
	}
	switch sy {
	case symbol.New("new"):
		prNew(m)
