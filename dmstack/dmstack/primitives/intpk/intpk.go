...
func prFromStr(m *machine.T) {
  ...
  if err != nil {
    m.Failf("'%v' bad integer.", tk.StringDraft())
  }
  pushInt(m, i)
}
...
// Returns a random number from 0 (inclusive) to i (exclusive)
//    m  : Virtual machine.
func prRnd(m *machine.T) {
	tk := m.PopT(token.Int)
	i, _ := tk.I()
  if i <= 0 {
    m.Failf("'%v' is <= 0.", tk.StringDraft())
  }
  pushInt(m, rand.Int63n(i))
}

// Returns division and module of num / den
//    m  : Virtual machine.
...
func Proc(m *machine.T) {
	tk, ok := m.PrgNext()
	if !ok {
		m.Fail("'int' procedure is missing")
	}
	sy, ok := tk.Sy()
	if !ok {
		m.Failf("Expected: 'int' procedure.\nActual  : %v.", tk.StringDraft())
	}
	switch sy {
	...
	default:
		m.Failf("'int' does not contains the procedure '%v'.", sy.String())
	}
}
