...
func prFromStr(m *machine.T) {
	...
	if err != nil {
		m.Failt("'%v' bad integer.", tk.StringDraft())
	}
	pushInt(m, i)
}
...
func prRnd(m *machine.T) {
	...
	i, _ := tk.I()
	if i <= 0 {
		m.Fail(machine.ERange(), "'%v' is <= 0.", tk.StringDraft())
	}
...
func prDiv(m *machine.T) {
	den := popInt(m)
	if den == 0 {
		m.Fail(machine.EMath(), "Division by 0")
	}
	num := popInt(m)
	pushInt(m, num/den)
	pushInt(m, num%den)
...
func Proc(m *machine.T, proc symbol.T) {
	...
	case symbol.New("not"):
		prNot(m)
	case symbol.New("lshift"):
		prLeft(m)
	case symbol.New("rshift"):
		prRight(m)
	...
	default:
		m.Failt("'int' does not contains the procedure '%v'.", proc.String())
	}
}
