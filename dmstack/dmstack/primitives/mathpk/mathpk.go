...
// Returns n round with scale decimals.
//    m  : Virtual machine.
...
func Proc(m *machine.T) {
	tk, ok := m.PrgNext()
	if !ok {
		m.Fail("'math' procedure is missing")
	}
	sy, ok := tk.Sy()
	if !ok {
		m.Failf("Expected: 'math' procedure.\nActual  : %v.", tk.StringDraft())
	}
	switch sy {
	...
	default:
		m.Failf("'math' does not contains the procedure '%v'.", sy.String())
	}
}
